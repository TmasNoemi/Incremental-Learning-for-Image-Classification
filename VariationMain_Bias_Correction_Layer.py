...
class iCarlNet(nn.Module):
    def __init__(self, n_classes, dictionary):
        ...
        self.val_old = val_old_dataloader
        self.val_new = val_new_dataloader 
    ...
    def train_initialize(self, dataloader):
      ...
      print('copia pesi fully connected classi 0 a 10:')
    ...
    def update_model(self, step):
      ...
      self.model.fc.weight.data[:step] = old_weight
    ...
    def train_fully_connected(self, dataloader, step):
      ...
      scheduler = optim.lr_scheduler.StepLR(optimizer, 20 , gamma=0.2)
    ...
    def train_bias_layer(self, step):

        self.model.cuda()
        cudnn.benchmark 
        start = time.time()
        
        print(f"Updating Bias parameters...")
        print(f'Parameters before update: alpha:{self.bias_layer.alpha.item():.4f} - beta:{self.bias_layer.beta.item():.4f}')

        bias_optimizer = optim.SGD(self.bias_layer.parameters(), lr=0.005)
        self.model.train(False)

        for epoch in range(250):

            running_loss = 0.0
            for old, new in zip(self.val_old[(step//10)-1],self.val_new[step//10]):

                bias_optimizer.zero_grad()

                old_imgs = Variable(old[0]).cuda()
                new_imgs = Variable(new[0]).cuda()

                new_logits = self.model(new_imgs)
                old_logits = self.model(old_imgs)
                outputs = self.bias_layer(new_logits)
                loss = self.criterion_dist(torch.sigmoid(outputs[:old_logits.size(0),:]), torch.sigmoid(old_logits))

                loss.backward()
                bias_optimizer.step()
                running_loss += loss.item() * old_imgs.size(0)

            del old_imgs, new_imgs
            torch.cuda.empty_cache()
            epoch_loss = running_loss/float(350) 
            print('Loss: {:.4f}'.format(epoch_loss))

        print(f'Bias parameters after: alpha: {self.bias_layer.alpha.item():.4f} - beta:  {self.bias_layer.beta.item():.4f}')
        print()  
    ...
    '''def create_exemplars(self, mini_batch, m):
      ...
      exemplars_set = random.sample(range(0,len(mini_batch)),m)
      self.exemplars.append(exemplars_set)'''
    ...
    def train(self, dataloader, step):
      ...
      for epoch in range(self.num_epochs):
            ...
            for images, labels in dataloader:            
                  ...
                  one_hot = one_hot.type_as(outputs)
                  _, preds = torch.max(outputs.data, 1)
                  unbiased_outputs = self.bias_layer(outputs[:,-10:])
                  outputs = torch.cat((outputs[:,:-10], unbiased_outputs),1)
...
for i in range(10,110,10):
  ...
  index     = [k for k,el in enumerate(trainset.targets) if el in x]                 #indici che vogliamo avere nel train batch
  idx_val   = random.sample(index, 350)
  #print(len(np.unique(np.array(trainset.targets)[idx_val])))
  idx_train = [el for el in index if el not in idx_val]
  batches_train.append(Subset(trainset, idx_train))                             
  val_new.append(Subset(trainset, idx_val))                                     
  
  index     = [k for k,el in enumerate(trainset.targets) if el in y]                
  idx_val   = random.sample(index, 350)
  #print(len(np.unique(np.array(trainset.targets)[idx_val])))
  idx_test  = [j for j, el in enumerate(testset.targets) if el in y]
  batches_test.append(Subset(testset, idx_test))
  val_old.append(Subset(trainset, idx_val))    
  idx_test1 = [j for j, el in enumerate(testset.targets) if el in y]
  batches_test_1.append(Subset(testset, idx_test1))
...
test_dataloaders_1 = [DataLoader(batch, batch_size=128, shuffle=True, num_workers=4, drop_last = True) for batch in batches_test_1]


val_old_dataloader = [DataLoader(batch, batch_size=128, shuffle=False, num_workers=4, drop_last = False) for batch in val_old]
val_new_dataloader = [DataLoader(batch, batch_size=128, shuffle=False, num_workers=4, drop_last = False) for batch in val_new]
...
K = 2000     # memory budget
m = K//10    # exempalrs per class
...
acc_new = [test_accuracy]
acc_old = [test_accuracy]

indice = 1
for idx in range(10,100,10):
    
  PRINT_CONF = True
  icarl.update_model(idx) 
  
  dataloader = icarl.combine_dataset_with_exemplars(batches_train[indice], mini_batches) 
...
#   test on old and new
  PRINT_CONF = False
  print('Accuracy on old classes...')
  new = icarl.Classify(test_dataloaders_1[indice])
  acc_new.append(new)
  print()
  print('Accuracy on new classes...')
  old = icarl.Classify(test_dataloaders[indice-1])
  acc_old.append(old)
