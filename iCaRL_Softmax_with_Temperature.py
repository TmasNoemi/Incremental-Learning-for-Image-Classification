    ...
    def train(self, dataloader, T):
      ...
      for epoch in range(self.num_epochs):
          ...
          for images, labels in dataloader:
              ...
              #Set Softmax with Temperature parameters 
              s = nn.LogSoftmax(dim=1) 
              q = s(outputs[:,0:self.n_classes]/T)
              p = (old_preds/T).long()

              loss_dist = self.criterion_NLL(q, p) 
              loss_class = self.criterion_CE(outputs, labels_map)
...
#import datasets and set transformations 
...
for i,idx in enumerate(range(10,100,10)):
  ...
  icarl.train(dataloader, 5) # we tested T = [5,2]
